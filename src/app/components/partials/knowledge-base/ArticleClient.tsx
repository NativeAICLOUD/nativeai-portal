...
export default function ArticleClient({ post, sections, related, readingTime, publishDate, category }: Props) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* ── Header ── */}
      <div className="relative overflow-hidden pt-28 pb-14" style={{ background: 'linear-gradient(145deg, #fff5ee 0%, #fdf0e8 30%, #fef6f0 60%, #fff8f2 100%)' }}>
        <div className="absolute top-0 right-0 w-[600px] h-[600px] pointer-events-none" style={{ background: 'radial-gradient(ellipse at 80% 20%, rgba(240,140,60,0.22) 0%, transparent 65%)', filter: 'blur(2px)' }} />
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] pointer-events-none" style={{ background: 'radial-gradient(ellipse at 20% 80%, rgba(232,154,120,0.14) 0%, transparent 65%)' }} />
        ...
        <div className="relative max-w-6xl mx-auto px-5 sm:px-8 lg:px-10">
          ...
          <nav className="flex items-center gap-1.5 text-[12px] text-[#0a0e1a]/40 mb-8">
            <Link href="/" className="hover:text-[#b86a30] transition-colors">Home</Link>
            <span>/</span>
            <Link href="/knowledge-base" className="hover:text-[#b86a30] transition-colors">Knowledge base</Link>
            <span>/</span>
          ...
          <div className="flex flex-col lg:flex-row lg:items-start gap-10 lg:gap-16">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 flex-wrap mb-5">
                ...
                <span className="px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-widest" style={{ background: 'rgba(240,160,96,0.14)', color: '#b86a30', border: '1px solid rgba(240,160,96,0.28)' }}>{category}</span>
              </div>
          ...
          <div className="mt-10 h-1 rounded-full overflow-hidden" style={{ background: 'rgba(0,0,0,0.07)' }}>
            <div className="h-full rounded-full transition-all duration-150" style={{ width: `${progress}%`, background: 'linear-gradient(90deg, #f0a060, #e89a78)' }} />
          </div>
        </div>
      </div>
            ...
            {mobileTocOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.22, ease: 'easeInOut' }} className="overflow-hidden">
                <nav className="flex flex-col gap-1 px-4 py-3" style={{ background: '#F7F7F7', borderTop: '1px solid rgba(0,0,0,0.05)', borderRadius: '0 0 12px 12px' }}>
                  {sections.map((s) => (
                    <button key={s.id} onClick={() => { scrollTo(s.id); setMobileTocOpen(false); }} className="text-left text-[13px] py-1.5 px-2 rounded-lg transition-all" style={activeSection === s.id ? { color: '#b86a30', fontWeight: 700 } : { color: '#0a0e1a', opacity: 0.5 }}>
                      {s.heading}
                    </button>
                  ))}
                </nav>
              ...
              <nav className="flex flex-col gap-0.5">
                {sections.map((s) => (
                  <button
                    ...
                    style={activeSection === s.id ? { color: '#b86a30', fontWeight: 700, background: 'rgba(240,160,96,0.10)' } : { color: '#0a0e1a', opacity: 0.45 }}
                  >
                    {s.heading}
                  </button>
                ))}
          ...
          <article className="flex-1 min-w-0 max-w-[720px]">
            {sections.map((s, idx) => (
              <div key={s.id} id={s.id} className="mb-14">
                <h2 className="text-[1.4rem] sm:text-[1.6rem] font-extrabold leading-snug mb-6" style={{ color: '#b86a30' }}>
                  {s.heading}
                </h2>
                    ...
                    {(s as RichSection).blocks.map((block, bi) => {
                          ...
                          {block.illustration ? (
                            ...
                            <div className="flex flex-col gap-4">
                              ...
                              <div className="flex items-center gap-4">
                                <div className="shrink-0 w-[100px] h-[72px] rounded-xl overflow-hidden" style={{ background: 'rgba(240,160,96,0.08)', border: '1px solid rgba(240,160,96,0.18)' }}>
                                ...
                                <div className="flex-1 flex items-center gap-0">
                                  ...
                                  <div className="w-2 h-2 rounded-full shrink-0" style={{ background: '#e89a78' }} />
                                </div>
                              </div>
                      ...
                      if (block.type === 'list') return (
                        block.ordered
                          ...
                          : <ul key={bi} className="flex flex-col gap-2 pl-2">{block.items.map((item, ii) => <li key={ii} className="flex gap-2.5 text-[15px] text-[#3a3a3a] leading-relaxed"><span className="mt-2 w-1.5 h-1.5 rounded-full shrink-0" style={{ background: '#e89a78' }} />{item}</li>)}</ul>
                      );
                      ...
                      if (block.type === 'cta') return (
                        <div key={bi} className="rounded-2xl px-6 py-7 flex flex-col sm:flex-row sm:items-center justify-between gap-4" style={{ background: '#0a0e1a' }}>
                          ...
                          <Link href={block.buttonUrl} className="shrink-0 inline-flex items-center gap-2 px-5 py-3 rounded-full text-sm font-semibold text-white transition-opacity hover:opacity-90 whitespace-nowrap" style={{ background: '#e89a78' }}>
                            {block.buttonText}
                  ...
                  <>
                    ...
                    {idx === 1 && (
                      <div className="mt-10 rounded-2xl px-6 py-7 flex flex-col sm:flex-row sm:items-center justify-between gap-4" style={{ background: '#0a0e1a' }}>
                        ...
                        <Link href="/schedule-call" className="shrink-0 inline-flex items-center gap-2 px-5 py-3 rounded-full text-sm font-semibold text-white transition-opacity hover:opacity-90 whitespace-nowrap" style={{ background: '#e89a78' }}>
                          Schedule a call
                          <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
                        </Link>
                      </div>
